...
import { getMigrations } from "../api/j2g-api";
import { MigrationWithExtraFields } from "../domain/migration";
...
interface DrawerExternalContentProps {
  children: React.ReactNode;
}

export default function MiniDrawer({ children }: DrawerExternalContentProps) {
  ...
  React.useEffect(() => {
    if (!dataFetched || refetchData) {
      getMigrations().then((data) => {
        setMigrations(
          data.map(calculateExtraMigrationFields)
        );
      });
  ...
  return (
    <Box sx={{ display: "flex" }}>
      <CssBaseline />
      <AppBar position="fixed" open={open} sx={{ backgroundColor: "#549d8a" }}>
        <Toolbar>
          <IconButton
            color="inherit"
